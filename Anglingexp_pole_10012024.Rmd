...
Angling_CPUE_data <- read_xlsx("AnglingClass_fulldata.xlsx",sheet=2)%>%
                        mutate(Fishing_experience_adjusted=
                                 case_when(Year==2011~Fishing_experience_score/3,
                                           Year==2020~Fishing_experience_score/4,
                                           Year==2024~Fishing_experience_score/4))
...
Full_CPUE_model_nb <- glmmTMB(N_fish~
                             ...
                             Fishing_day*Fishing_experience+
                             (1|Year)+(1|Angler)+(1|Location),
...
Sub_CPUE_model_nb <- glmmTMB(N_fish~
                             Fishing_experience_adjusted*Hook_size+Session_ID+
                             Fishing_day*Fishing_experience+
                             (1|Year)+(1|Angler)+(1|Location),
                             family = nbinom2, 
...
## Rolled category
ggplot(Rolled_Angling_CPUE_data, aes(x = as.factor(Fishing_experience_adjusted), y = CPUE, fill = as.factor(Fishing_experience_adjusted), color = as.factor(Fishing_experience_adjusted))) +
  ...
  # Manually set fill colors for violins (optional customization)
...
ggplot(Rolled_Angling_CPUE_data, aes(x = Fishing_experience, y = CPUE,fill=Fishing_experience)) +
...
N_species~Fishing_experience+Hook_size_quant+
...
AnglingClass_size_data_resricted <-read_excel("AnglingClass_fulldata.xlsx", sheet=1)%>%
   mutate(Date=as.Date(Date))%>%
   filter(!Date %in%  ymd(c("2020-09-02", "2020-09-03")))%>%
   mutate(log_Length=log(Length))%>%
   mutate(Fishing_experience_adjusted=
...
Full_size_model<-lmer(log_Length~Fishing_experience_adjusted*Hook_size+Session_ID+
                             Fishing_day*Fishing_experience+
                             (1|Year)+(1|Angler)+(1|Location),
...
Sub_size_model<-lmer(log_Length ~Fishing_experience_adjusted*Hook_size+Session_ID+
                             Fishing_day*Fishing_experience+
                             (1|Year)+(1|Angler)+(1|Location),
                      data=AnglingClass_size_data_removed)
...
#########consideration of large individuals

### FUll model for size analysis
Full_size_model_restricted<-lmer(log_Length~Fishing_experience_adjusted*Hook_size+Session_ID+
                             Fishing_day*Fishing_experience+
                             (1|Year)+(1|Angler)+(1|Location),
...
##sub model for size analysis
Sub_size_model_restricted<-lmer(log_Length ~Fishing_experience_adjusted*Hook_size+Session_ID+
                             Fishing_day*Fishing_experience+
                             (1|Year)+(1|Angler)+(1|Location),
                      data=AnglingClass_size_data_removed%>%filter(Species!="Bream"))
...
Full_model_bleeding<-glmer(data=AnglingClass_bleeding_data,
                       Bleeding~Fishing_experience*log_Length+Hook_size_quant*log_Length+(1|Year),family = binomial(link = "logit"))


# Full_model_bleeding_2<-glmer(data=AnglingClass_bleeding_data,
#                        Bleeding~Hook_size_quant+
#                         Fishing_experience_adjusted*log_Length+Hook_size_quant*log_Length+(1|Year),family = binomial(link = "logit"))
# 

summary(Full_model_bleeding)
Anova(Full_model_bleeding)

# summary(Full_model_bleeding_2)
# Anova(Full_model_bleeding_2)
...
count_data_Bleeding <- AnglingClass_bleeding_data%>%
  group_by(Fishing_experience, Treatment_Experience_Bleeding, Year) %>%
...
full_model_hooking_depth<-glmer(data=AnglingClass_hooking_data,
                  Hooking_depth~Fishing_experience*log_Length+Hook_size_quant*log_Length+(1|Year),
                     family = binomial(link = "logit"))

#sub model to check
full_model_hooking_depth2<-glmer(data=AnglingClass_hooking_data,
                  Hooking_depth~Fishing_experience_adjusted*log_Length+Hook_size_quant*log_Length+(1|Year), family = binomial(link = "logit"))


summary(full_model_hooking_depth)
summary(full_model_hooking_depth2)
Anova(full_model_hooking_depth)
Anova(full_model_hooking_depth2)


# sjPlot::plot_model(full_model_hooking_depth1, type = "pred",
#                    terms = c("Fishing_experience","Year"))
